...
const ProductService = () => {
  ...
  const [items,              setItems]              = useState(INITIAL_ITEMS); // ✅ added setItems
  const [showModal,          setShowModal]          = useState(false);
  const [sidebarActive,      setSidebarActive]      = useState("Product & Service");
  const [mobileSidebarOpen,  setMobileSidebarOpen]  = useState(false);

  const handleAddNew     = () => setShowModal(true);                               // ✅ only once
  const handleAddProduct = (newItem) => setItems(prev => [...prev, newItem]);
  ...
  return (
    <div className="relative flex bg-gray-50 min-h-screen overflow-hidden">
      ...
      {/* ✅ Modal inside return, before closing div */}
      {showModal && (
        <AddNewClientModal
