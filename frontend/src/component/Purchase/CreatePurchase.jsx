...
export default function CreatePurchase({ onBack, onSave, onSaveDraft, saving = false }) {
  ...
  const [vendor, setVendor]             = useState({ name: "", email: "", phone: "", gst: "", address: "" });
  ...
  // ── Payload shape matches purchaseController expectations ────────────────────
  const payload = () => ({
    purchaseNo,
    date,
    vendor:  selectedVendor,   // { name, email, phone, gst, address }
    items,                     // [{ name, description, hsn, qty, rate, amount }]
    note,
    terms,
    total,
  });
  ...
  const canSave = items.length > 0 && items.every(i => i.name.trim());
    ...
    <div className="flex-1 bg-gray-50 flex flex-col overflow-auto">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-6 py-3 gap-3">
        <button
          onClick={onBack}
          disabled={saving}
          className="flex items-center gap-2 text-gray-600 hover:text-gray-900 font-medium text-sm disabled:opacity-50"
        >
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-amber-400 hover:bg-amber-500 text-gray-900 font-semibold text-sm px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {saving ? (
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium text-sm px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v14a2 2 0 01-2 2z" />
            </svg>
        ...
        <div className="flex-1 min-w-0 p-4 sm:p-6 space-y-6">
          ...
          <div>
            ...
            {!canSave && items.length > 0 && (
              <p className="text-xs text-red-400 mt-1.5">Please fill in all item names before saving.</p>
            )}
          </div>
          ...
          <div className="p-4 sm:p-5 space-y-5 lg:sticky lg:top-[57px]">
            ...
            <div className="space-y-2">
              <button
                onClick={() => onSave?.(payload())}
                disabled={saving || !canSave}
                className="w-full flex items-center justify-center gap-2 bg-amber-400 hover:bg-amber-500 text-gray-900 font-bold text-sm py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                  <polyline points="14 2 14 8 20 8" />
                </svg>
                {saving ? "Saving…" : "Generate Purchase"}
              </button>
              <button
                onClick={() => onSaveDraft?.(payload())}
                disabled={saving}
                className="w-full flex items-center justify-center gap-2 border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium text-sm py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v14a2 2 0 01-2 2z" />
                </svg>
                Save Draft
