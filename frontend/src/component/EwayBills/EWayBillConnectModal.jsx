          ...
          <div key={label} className="flex items-center">
            <div className="flex items-center gap-1.5">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0
                ${done || active ? "bg-amber-400 text-white" : "bg-gray-200 text-gray-500"}`}>
                {done
...
async function connectToGSP({ gspUsername, gspPassword }) {
  const token = localStorage.getItem('token') // or wherever you store it

  const res = await fetch(`${API_BASE}/api/ewaybill/connect`, {
    method: "POST",
    headers: {
...
// ── Modal ─────────────────────────────────────────────────────────────────────
export default function EWayBillConnectModal({ onClose, onConnected }) {
  ...
  useEffect(() => { setApiError(""); }, [username, password]);
  ...
  const handleProceed = async () => {
    ...
    if (step === 2) {
      ...
      setApiError("");
      try {
        await connectToGSP({ gspUsername: username.trim(), gspPassword: password });
        setStep(3);
        onConnected?.();
      } catch (err) {
          ...
          {step === 1 && (
            <div className="space-y-4 text-sm text-gray-700">
              <p className="leading-relaxed">
                ...
                Provider (GSP) on the <span className="font-semibold">E-way Bill Portal</span>.
              </p>
              <div>
                ...
                <ol className="space-y-2 text-gray-600 list-none">
                  <li>1. Login to the{" "}
                    <a href="https://ewaybillgst.gov.in" target="_blank" rel="noreferrer"
                      className="text-blue-500 hover:underline font-medium">E-way Bill Portal</a>
                  </li>
                  <li>2. Click <span className="font-semibold">Registration</span> in the left menu → select <span className="font-semibold">For GSP</span></li>
                  <li>3. Click <span className="font-semibold">Send OTP</span> and verify</li>
                </ol>
              </div>
              <div>
                <p className="font-bold text-gray-800 mb-2">STEP - 2</p>
                <ol className="space-y-2 text-gray-600 list-none">
                  <li>1. Click the <span className="font-semibold">Add / New</span> button</li>
                  <li>2. Select <span className="font-semibold">Tera Software Limited</span> from the GSP Name dropdown</li>
                  <li>3. Enter a 3-letter Suffix ID and a password</li>
                  <li>4. Re-enter the Username &amp; Password, then click <span className="font-semibold">ADD</span></li>
                </ol>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <p className="text-sm text-gray-700 leading-relaxed">
                Enter the GSP credentials you just created on the E-way Bill Portal.
                We'll verify them using your business GSTIN automatically.
              </p>
              ...
              <p className="text-xs text-gray-400 pt-1">
                Your credentials are stored securely and used only to generate E-way Bills.
              </p>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center py-10 text-center space-y-4">
              ...
              <p className="text-sm text-gray-500 leading-relaxed">
                Your GSP credentials have been verified and saved. You can now generate E-way Bills.
              </p>
              <button onClick={onClose}
          ...
          <div className="px-6 py-4 border-t border-gray-100 shrink-0 flex items-center gap-3">
            {step === 2 && (
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold
                           border border-gray-200 text-gray-600 hover:bg-gray-50 active:scale-95
            ...
            >
              {loading ? (
                ...
                <>
                  {step === 1 ? "Proceed to E-way Bill GSP Login" : "Connect to E-way Bill Portal"}
                  <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
